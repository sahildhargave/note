    ...
    return BlocBuilder<NoteBloc, NoteState>(
      builder: (context, state) {
        if (state is NoteListState) {
          ...
          return IconButton(
              onPressed: () async {
                await notedbHelper.updateIsFavorite(
                    state.noteList[widget.index].noteId!,
                    state.noteList[widget.index].isFavourite == 0 ? 1 : 0);
                noteBloc.add(TumNotlarEvent(categoryId: -1));
              },
              icon: state.noteList[widget.index].isFavourite == 1
                  ? Icon(
                      Icons.star,
                      color: Colors.yellow,
                      size: 27,
