      ...
      return Obx(
        () => AnimatedContainer(
          decoration: BoxDecoration(
            ...
            color: Get.isDarkMode ? Color(0xff383838) : Color(0xffe8e8e8),
          ),
          duration: Duration(seconds: 1),
          width: double.infinity,
          height: (getxController.todoMap.length).toDouble() * 50 > 270
              ? 270
              : (getxController.todoMap.length).toDouble() * 50,
          child: ListView(
            children: [
                ...
                Container(
                    ...
                    child: Row(
                      children: [
                        Checkbox(
                            activeColor:
                                Color(0xff006c8d), //0xffd1c4e9  0xff00626f
                            value: i.value == 1 ? true : false,
                            shape: CircleBorder(),
                            onChanged: (bool? value) async {
                              getxController.todoMap[i.key] =
                                  (value == true) ? 1 : 0;
                            }),
                        Expanded(
                          child: Text(
        ...
        builder: (context, state) {
          if (state is NoteListState) {
            Map<String, int> todoList =
                state.noteList[indexOfNote!].todoList ?? {};
            return AnimatedContainer(
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(15),
                color: Get.isDarkMode ? Color(0xff383838) : Color(0xffe8e8e8),
              ),
              margin: todoList.length > 0
                  ? EdgeInsets.symmetric(vertical: 5)
                  : null,
              duration: Duration(seconds: 1),
              width: double.infinity,
              height: (todoList.length).toDouble() * 50 > 270
                  ? 270
                  : (todoList.length).toDouble() * 50,
              child: ListView(
                children: [
                        ...
                        child: Row(
                          children: [
                            Checkbox(
                                ...
                                shape: CircleBorder(),
                                onChanged: (bool? value) async {
                                  await notedbHelper.updateTodo(
                                      state: value == true ? 1 : 0,
                                      noteId:
                                          state.noteList[indexOfNote!].noteId,
                                      todoTitle: i.key);
