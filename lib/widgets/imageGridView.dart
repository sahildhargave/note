      ...
      return Container(
        ...
        child: GridView.builder(
            ...
            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: imageList.length > 4 ? 2 : 1,
            ),
            ...
            itemBuilder: (context, index) {
              return Container(
                ...
                child: Image.file(
                  File(StaticValues.imageStringList[index]),
                  fit: BoxFit.fill,
                ),
              );
            }),
      ...
      return BlocBuilder<NoteBloc, NoteState>(
        builder: (context, state) {
          if (state is NoteListState) {
            List allImageList =
                state.noteList[indexOfNote!].imageList!.values.toList();
            if (state.noteList[indexOfNote!].imageList!.length > 0) {
              return Container(
                margin: EdgeInsets.symmetric(vertical: 5),
                height: allImageList.length > 0
                    ? (allImageList.length <= 4 ? 100 : 200)
                    : 0,
                child: GridView.builder(
                    scrollDirection: Axis.horizontal,
                    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                    ...
                    itemBuilder: (context, index) {
                      int imageId = state.noteList[indexOfNote!].imageList!.keys
                          .toList()[index];
                              ...
                              Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                      builder: (context) => ImageViewPage(
                                            path: allImageList[index],
                                            imageId: imageId,
                                          )));
                            },
                            child: Container(
                              child: Image.file(
                                File(allImageList[index]),
                                fit: BoxFit.fill,
                              ),
                            ),
                          ),
