...
class NoteDatabaseHelper {
  Future<Database> noteDatabase() async {
    print("The note database has been copied");
  ...
  Future<List<Note>> getNotes(
    ...
    for (var notMap in noteMapList) {
      List<Map<String, dynamic>> imageMapList = await database
          .rawQuery("SELECT * FROM images WHERE noteId = ${notMap["noteId"]}");
      List<Map<String, dynamic>> todoMapList = await database.rawQuery(
          "SELECT * FROM noteTodo WHERE noteId = ${notMap["noteId"]}");
  ...
  Future<double> getFontsize() async {
    ...
    var x = await _db
        .rawQuery("SELECT * FROM settings WHERE whichSetting ='fontSize'");
  ...
  Future<List<Note>> getFavoriteNotes() async {
    ...
    noteList = noteList.where((element) => element.isFavourite == 1).toList();
    return noteList;
  }
  ...
  Future<void> updateNoteTitle(int id, String title) async {
    ...
    Database database = await noteDatabase();
    await database
        .rawQuery("UPDATE notes SET noteTitle = '$title' WHERE noteId ='$id'");
  }
  ...
  Future<void> updateNoteCategory(int id, int categoryId) async {
    ...
    Database database = await noteDatabase();
    await database.rawQuery(
        "UPDATE notes SET noteCategoryId = '$categoryId' WHERE noteId ='$id'");
  }

  Future<void> updateIsFavorite(int id, int value) async {
    Database database = await noteDatabase();
    await database
        .rawQuery("UPDATE notes SET isFavorite = '$value' WHERE noteId ='$id'");
  }
  ...
  Future<void> updateIsLocked(int id, int locked) async {
    ...
    locked == 1
        ? await database
            .rawQuery("UPDATE notes SET isLocked = '1' WHERE noteId ='$id'")
        : await database
            .rawQuery("UPDATE notes SET isLocked = '0' WHERE noteId ='$id'");
  }
  ...
  Future<int> insertRemainder(Remainder remainder) async {
    ...
    int id = 0;
    await _db
        .insert("remainders", remainder.toMap(),
            conflictAlgorithm: ConflictAlgorithm.replace)
        .then((value) => id = value);
  ...
  Future<List<Remainder>> getRemainders(int noteId) async {
    Database database = await noteDatabase();
    List<Map<String, dynamic>> remainderMapList = await database
        .rawQuery("SELECT * FROM remainders WHERE noteId = '$noteId'");
  ...
  Future<void> insertTodo(int noteId, Map<dynamic, dynamic> todoMap) async {
    ...
    todoMap.forEach((key, value) async {
      await _db.insert(
          "noteTodo", {"noteId": noteId, "title": key, "isDone": value},
          conflictAlgorithm: ConflictAlgorithm.replace);
    });
  }
  ...
  Future<void> deleteTodo({required String todoTitle, required noteId}) async {
    Database _db = await noteDatabase();
    await _db.rawQuery(
        "DELETE FROM noteTodo WHERE title ='$todoTitle' AND noteId ='$noteId'");
  }

  Future<void> updateTodo(
      {required int state, required noteId, required String todoTitle}) async {
    Database _db = await noteDatabase();
    await _db.rawQuery(
        "UPDATE noteTodo SET isDone = '$state' WHERE title ='$todoTitle' AND noteId ='$noteId'");
  }

  Future<void> setFontsize({required double fontSize}) async {
    Database _db = await noteDatabase();
    await _db.rawQuery(
        "UPDATE settings SET setting = '${fontSize.toInt()}' WHERE whichSetting = 'fontSize'");
  }
  ...
  Future<void> deleteCategory({required int categoryId}) async {
    Database _db = await noteDatabase();
    await _db
        .rawQuery("DELETE FROM categories WHERE categoryId ='$categoryId'");
    await _db.rawQuery(
        "UPDATE notes SET noteCategoryId = ${1} WHERE noteCategoryId = '$categoryId'");
  }
  ...
  Future<void> moveNotesToTrash(List<int> idList) async {
    ...
    for (var id in idList) {
      await _db
          .rawQuery("UPDATE notes SET isDeleted ='${1}' WHERE noteId ='$id'");
    }
  }
  ...
  Future<void> recoverNotefromTrash(int noteId) async {
    Database _db = await noteDatabase();
    await _db
        .rawQuery("UPDATE notes SET isDeleted ='${0}' WHERE noteId ='$noteId'");
  }

  Future<void> removeLockfromAllNotes() async {
    Database database = await noteDatabase();
    await database
        .rawQuery("UPDATE notes SET isLocked = '0' WHERE isLocked = '1'");
  }
}
